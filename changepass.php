...
if(isset($_POST["submit"])){
	if(!empty($_POST['password']) && !empty($_POST['email'])){
		...
		$result = mysqli_query($mysqli, "UPDATE google_users SET password='$changepass',pass_stat='1' WHERE email='$changeemail'");
...
<div class="auth-top-up">Change Password</div>

<div class="authup-form-control">

  <form action='<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>' method='post' class="authup-form">

    <input type="password" name="old_password" placeholder="Enter Default Password">
    <input type="password" name="password" placeholder="Enter New Password">
    <input type="hidden" name="email" value="<?php echo $email_pass; ?>"> 
